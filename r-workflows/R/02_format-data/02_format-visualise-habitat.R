...
habitat <- readRDS(paste0("r-workflows/data/staging/",
                           name, "_habitat.rds")) %>%
...
tidy.relief <- readRDS(paste0("r-workflows/data/staging/",
                               name, "_relief.rds")) %>%
  uncount(number) %>%
...
plot.relief <- readRDS(paste0("r-workflows/data/staging/",
                               name, "_relief.rds")) %>%
...
saveRDS(tidy.habitat, file =
            paste("r-workflows/data/tidy/", 
                  paste(name,"tidy-habitat.rds", sep = "_"),
                  sep = "/"))
...
min.lon <- min(as.numeric(tidy.habitat$longitude_dd))
min.lat <- min(as.numeric(tidy.habitat$latitude_dd))
max.lon <- max(as.numeric(tidy.habitat$longitude_dd))
max.lat <- max(as.numeric(tidy.habitat$latitude_dd))
...
pie.chart <- leaflet() %>%                                                      # Create a leaflet plot
  ...
  addMinicharts(plot.habitat$longitude_dd, plot.habitat$latitude_dd,  # Add a spatial minichart using spatial information from the metadata
...
if (nrow(overzero)) {                                                           # Add spatial bubble plots if the data is greater than zero
  bubble.plot <- bubble.plot %>%
    addCircleMarkers(data = overzero, lat = ~ latitude_dd, lng = ~ longitude_dd,      # Add the bubble plots
...
if (nrow(equalzero)) {                                                          # Add spatial bubble plots if the data is equal to zero
  bubble.plot <- bubble.plot %>%
    addCircleMarkers(data = equalzero, lat = ~ latitude_dd, lng = ~ longitude_dd,     # Add the bubble plots
