...
plot_transformations(pred.vars = pred.vars, dat = tidy.count)
```
...
use.dat <- as.data.frame(tidy.count)
out.all = list()
var.imp = list()
```
...
```{r}
count <- readRDS(here::here(paste0("r-workflows/data/tidy/", 
                      name, "_tidy-count.rds"))) %>%
  clean_names() %>%
  glimpse()
```
...
```{r}
length <- readRDS(here::here(paste0("r-workflows/data/tidy/", 
                        name, "_tidy-length.rds"))) %>%
  clean_names() %>%
  dplyr::select(-status) %>%
  glimpse()
