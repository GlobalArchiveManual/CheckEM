...
Plot the individual predictors to assess if any transformations are necessary
```{r transformations, warning = FALSE, fig.width = 7, fig.height = 6}
par(mfrow = c(3, 2))
for (i in pred.vars) {
  x <-dat[ , i]
  ...
  hist((x), main = paste(i))
  plot((x), main = paste(i))
  hist(sqrt(x), main = paste0("sqrt(",i, ")"))
  plot(sqrt(x), main = paste0("sqrt(",i, ")"))
  hist(log(x + 1), main = paste0("log(",i, "+ 1)"))
  plot(log(x + 1), main = paste0("log(",i, "+ 1)"))
}
```
...
Add the output directory for model outputs, and set up the R environment for model selection
```{r environment-setup}
...
Run the full subset model selection process. This loop has been adapted from @beckyfisher/FSSgam, and examples and documentation is available on GitHub and in Fisher, R, Wilson, SK, Sin, TM, Lee, AC, Langlois, TJ. A simple function for full-subsets multiple regression in ecology with R. Ecol Evol. 2018; 8: 6104–6113. https://doi.org/10.1002/ece3.4134
...
```{r aggregate, fig.width = 7, fig.height = 4}
...
# Create a column for the 'dominant' habitat and tidy the names for plotting
```{r dominant-tag}
...
```{r save-final}
saveRDS(preddf_a, file = here::here(paste0("r-workflows/model output/habitat/", 
